...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Appearance */}
      <Card>
        <CardHeader title="Appearance" subtitle="Switch between light and dark — easier on the eyes at night" />
        <CardBody>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="text-[15px] text-slate-700 dark:text-slate-200">Theme</span>
            <AppearanceControl />
          </div>
        </CardBody>
      </Card>
      ...
      <Card>
        ...
        <CardBody className="divide-y divide-stone-100 py-0 dark:divide-slate-800">
        ...
        <>
            ...
            <CardBody>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="font-medium text-slate-900 dark:text-slate-100">{currentUser.full_name}</div>
                  <div className="text-[13px] text-slate-500 dark:text-slate-400">{currentUser.email} · Owner / Admin</div>
                </div>
                <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[12px] font-medium text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">You</span>
              </div>
          ...
          <Card>
            ...
            <CardBody className="space-y-3">
              {INTEGRATIONS.map((i) => (
                <div key={i.name} className="flex items-center justify-between gap-3 rounded-xl border border-stone-200 px-4 py-3 dark:border-slate-800">
                  <div>
                    <div className="font-medium text-slate-900 dark:text-slate-100">{i.name}</div>
                    <div className="text-[13px] text-slate-500 dark:text-slate-400">{i.desc}</div>
                  </div>
                  <span className="flex items-center gap-2 text-[13px] text-slate-400 dark:text-slate-500">
                    <span className="h-2 w-2 rounded-full bg-slate-300 dark:bg-slate-600" /> Not connected
                  </span>
                </div>
              ))}
            </CardBody>
            ...
            <CardBody className="space-y-3">
              {myProperties.map((p) => (
                <Link
                  ...
                  className="flex items-center gap-3 rounded-xl border border-stone-200 px-4 py-3 transition-colors hover:bg-stone-50 dark:border-slate-800 dark:hover:bg-slate-800/60"
                >
                  ...
                  <div>
                    ...
                    <div className="text-[13px] text-slate-500 dark:text-slate-400">{p.address_line1}, {p.city}, {p.state}</div>
                  </div>
                </Link>
              ))}
            </CardBody>
            ...
            <Card>
              ...
              <CardBody>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-display text-lg text-slate-900 dark:text-slate-100">{manager.full_name}</div>
                    <div className="text-sm text-slate-500 dark:text-slate-400">{manager.email}</div>
                    {manager.phone ? <div className="text-sm text-slate-500 dark:text-slate-400">{manager.phone}</div> : null}
                  </div>
                </div>
              </CardBody>
            </Card>
