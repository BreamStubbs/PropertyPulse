...
export default function CalendarPage() {
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col items-start justify-between gap-3 rounded-2xl border border-sky-200 bg-sky-50 px-5 py-4 sm:flex-row sm:items-center">
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-sky-100 text-sky-600">
            <Icon.Sparkle className="h-5 w-5" />
          </span>
          <div>
            <p className="font-medium text-slate-900">Connect your Airbnb or VRBO calendar</p>
            <p className="text-sm text-slate-500">Auto-sync reservations from your listing’s calendar feed — coming soon.</p>
          </div>
        </div>
        <button
          disabled
          className="cursor-not-allowed rounded-xl border border-sky-300 bg-white px-4 py-2 text-sm font-medium text-sky-400"
        >
          Connect — soon
        </button>
      </div>
      ...
      {selectedDay ? (
        <div className="mt-5 rounded-2xl border border-stone-200 bg-white p-5 shadow-card">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-lg text-slate-900">{formatDate(selectedDay.key)}</h3>
            {isAdmin ? (
          ...
          {selectedDay.events.length === 0 ? (
            ...
            <ul className="mt-3 space-y-3">
              {selectedDay.events.map((e) => (
                <li key={e.id} className="flex items-start justify-between gap-3 border-b border-stone-100 pb-3 last:border-0 last:pb-0">
                  <div>
